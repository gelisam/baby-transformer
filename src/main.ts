...
function processMessage(schedule: Schedule, msg: Msg): void {
  switch (msg.type) {
    ...
    case "ReinitializeModel": {
      ...
      model.reinitializeModel(schedule, m.numLayers, m.neuronsPerLayer, m.inputFormat, m.vocabSize);

      // 2. Generate new data and push to modules via message (dataset.ts)
      dataset.reinitializeModel(schedule, m.numLayers, m.neuronsPerLayer, m.inputFormat, m.vocabSize);
      ...
      uiControls.reinitializeModel(schedule, m.numLayers, m.neuronsPerLayer, m.inputFormat, m.vocabSize);
