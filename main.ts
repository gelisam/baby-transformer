...
function generateData(): TrainingData {
  ...
  // Convert each input from token numbers to embedded vectors
...
function drawNetworkArchitecture(): void {
  ...
  for (let i = 0; i < layers.length; i++) {
    ...
    if (i === 0) {
      ...
      ctx.lineWidth = 4;
      if (i === 1) {
        // Embedding layer (no activation, just lookup)
        ctx.strokeStyle = '#90EE90'; // LightGreen for embedding
        ctx.beginPath();
        ...
        // Linear layer before unembedding
